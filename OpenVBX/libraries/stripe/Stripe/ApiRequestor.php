...
class Stripe_ApiRequestor
{
  ...
  public function handleApiError($rbody, $rcode, $resp)
  {
    if (!is_array($resp) || !isset($resp['error']))
      return("Stripe_ApiError: Invalid response object from API: $rbody (HTTP response code was $rcode)");
    $error = $resp['error'];
    switch ($rcode) {
    case 400:
    case 404:
      return("Stripe_InvalidRequestError:".(isset($error['message']) ? $error['message'] : null).(isset($error['param']) ? $error['param'] : null));
    case 401:
      return("Stripe_AuthenticationError:".(isset($error['message']) ? $error['message'] : null));
    case 402:
      return("Stripe_CardError:".(isset($error['message']) ? $error['message'] : null).
				  (isset($error['param']) ? $error['param'] : null).
				  (isset($error['code']) ? $error['code'] : null));
    default:
      return("Stripe_ApiError:".(isset($error['message']) ? $error['message'] : null));
    }
  }
  ...
  private function _interpretResponse($rbody, $rcode)
  {
    ...
    if ($rcode < 200 || $rcode >= 300) {
	      ...
	      throw new Exception("stripe error: ", 1, $resp);
	    }
    }
    return $resp;
  }
