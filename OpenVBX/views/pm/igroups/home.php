...
.container .brand {
  ...
  <body>
	...
	<div class="container">
		<div class="content">
			<?php if (isset($page) && $page == "group") {?>
				<?php if (isset($grouppage)) {?>
			<div class="page-header">
				<h1><?= $grouppage['title']?> <small><?= $grouppage['description']?></small></h1>
				<div class="alert-message <?= $grouppage['alert-type']?>">
				  <p><?= $grouppage['alert-message']?></p>
				</div>
			</div>
				<?php } ?>
			<?php } ?>
			<div class="row">
				...
				$col_id = 0;
				for ($col_id=0; $col_id < $horizontal_spans['cols']; $col_id++)
				{
					// remove module from module to load list
					$remove_mod = true;
					$mod_doshow = false;
					//echo $horizontal_spans['spans'][$col_id];
				?>
				<div class="<?= $horizontal_spans['spans'][$col_id]; ?>">
					<?php foreach ($modules as $module_name => $modules_pos) {
						...
						if (!($modules_pos == "left" && $col_id % 2 == 0)) 
						{ 
							$remove_mod = false;
							//$mod_doshow = false;
						...
						//echo "mod_doshow: $mod_doshow module_name: $module_name modules_pos: $modules_pos";
						if ($mod_doshow) { ?>
						
						<?php if ($module_name == "feed") {?>
							<h2 name='feed' id='feed'>Feed</h2>
							<p><span class="label">9/28/11 &nbsp;&nbsp;7:00AM</span> <span class="label important">Soccer Practice Group</span> Varsity Team Practice moved to Soldiers Field</p>
							<p><span class="label">9/28/11 11:00AM</span> <span class="label important">My Fall 2011 Something Group</span> Meeting in Salt</p>
							<p><span class="label">9/28/11 &nbsp;&nbsp;4:00AM</span> <span class="label important"><a href="">My Fall 2011 Something Group</a></span> Exploratory Committee in Pepper</p>
						<?php } else if ($module_name == "calendar") {
								if ($modules_pos == "right" && $col_id % 2 == 1) { ?>
									<h2>Calendar</h2>
							<h2 name='calendar' id='calendar'>Calendar</h2>
							
							<div id='calendar'></div>
							
							<?php  } else { $remove_mod = false; } ?>
						<?php  } else { ?>
								<h2><?= $module_name ?></h2>
