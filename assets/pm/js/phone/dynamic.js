...
//		console.log(button_txt);
	...
	if (module == "files")
	{
		...
		$.getJSON('/phone/slim?module='+module+'&group_id='+groupid+'&user_id='+userid+'&inGroupView='+groupviewmode+'', function(data) {
			if (module == "metrics")
			{
				document.getElementById('jsin').innerHTML = '<div id="container" style="width: 800px; height: 400px; margin: 0 auto"></div>';
				eval(data.result);
				document.getElementById('pane1content').innerHTML = "";
...
function backToDashboardView(user_id) 
{
	...
	$.getJSON('/phone/dashboard?user_id='+user_id+'', function(data) {
