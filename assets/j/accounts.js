...
function stripeResponseHandler(status, response) {
    if (response.error) {
		...
		transactionData.tenant_id							=	$('#tenant_id').val();
		transactionData.billing_addressline1				=	$('#billing_addressline1').val();
		transactionData.billing_addresszip					=	$('#billing_addresszip').val();
		transactionData.billing_name						=	$('#billing_name').val();
		transactionData.paymentFail							=	paymentFail;
		transactionData.stripeToken							=	stripeToken;
		transactionData.purchase_cardtype					=	$('#purchase_cardtype').val();
		transactionData.purchase_cardid						=	$('#purchase_cardid').val();
		transactionData.purchase_planid						=	$('#purchase_planid').val();
		transactionData.purchase_total_amount				=	$('#purchase_total_amount').val()*100;
	
		$.postJSON('charge/chargeToken', transactionData, function(data) {
			transactionStatus = data.result;
			if (transactionStatus == "-1" || transactionStatus == "1")
			{
				$(".payment-errors").html("<b><font color='navy'>Charge Successful!</font> Your additional resources are now available!</b>");
			} else {
				$(".payment-errors").html("<b><font color='red'>Charge Failed!</font> Please try again or use a different card.</b>");
			}
			
			$('#dashboard_smss_available').val(data.sms);
			$('#dashboard_minutes_available').val(data.minutes);
			$('#dashboard_tfminutes_available').val(data.tfminutes);
			$('#dashboard_balance').val(data.balance);
		});
    }
}
...
$(document).ready(function() {
	...
	var dialogFundsAdd = $('#dialog-funds-add');
	$('#dialog-funds-add').attr('title', 'Add Funds to Your Account');
	$('#dialog-funds-add').dialog({ 
		autoOpen: false,
		width: 650,
		buttons: {
			'Add': function() { 
				var dialog = $(this);
				var params = $('#dialog-funds-add').values();
				
				var amount = $('#purchase_total_amount').val()*100; //amount you want to charge in cents
				if ($('#purchase_cardtype').val() != "existing" || stripeToken != false)
				{
					   Stripe.createToken({
				        number: $('#billing_card-number').val(),
				        cvc: $('#billing_card-cvc').val(),
				        exp_month: $('#billing_card-expiry-month').val(),
				        exp_year: $('#billing_card-expiry-year').val(),
				        address_line1: $('#billing_addressline1').val(),
				        address_zip: $('#billing_addresszip').val(),
				...
				if (stripeToken != false && transactionStatus != 0)
				{
					$('button', dialog).prop('disabled', false);
				}
				
				if (transactionData.purchase_cardtype == "existing")
				{
					$.postJSON('charge/chargeToken', transactionData, function(data) {
						transactionStatus = data.result;
						if (transactionStatus == "-1" || transactionStatus == "1")
...
//				history.go(0);
	...
	$('#button-funds-add').click(function(e) {
		showFundsAdd(null);
		return false;
	});
...
function showFundsAdd(data) {
	var dialog = $('#dialog-funds-add');
	...
	setTimeout(function() {
		$('#dialog-add-funds input:first').focus();
	}, 100);
